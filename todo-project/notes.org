...
- [-] Line 2: Selected List.
  ...
  - [ ] Clicking on a selected tag will:
    - [ ] remove it from the selected list
    - [ ] remove it from association with the todo item
...
- [ ] can go under existing filter
  - [ ] or maybe to the right of the existing filter?
- [ ] selecting a tag makes a search happen
  - [ ] search is conducted against the lookup "table" and matches are displayed the same Filter search results are
