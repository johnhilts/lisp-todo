...
**** TODO this whole thing probably needs to be re-written
**** TODO Client
- [-] App Settings should store the current filter tag selection!
  - [X] Get basics into place
    - See the =render-todos-filtered-by-tags= function.
    - *selected-filter-tag-todo-ids*
    - *filter-tag-match-type* "all/any"
  - [ ] Need to handle page refresh better
  - =render-todo-filter= <-- where "filter-tag-candidates-selected" is rendered
...
- [ ] BUG indecisive tags while adding
  - add 4 tags
  - Remove the middle 2
  - Try to add one back
  - Completely random tag now selected
- [X] BUG some of the ALL filter logic is wrong
  - [X] Home all / any should be the same, but ALL has 1 less than ANY - why?!?
  - [X] +try with bread and water - should have some entries, but it shows zero+
    - Can't reproduce this one
- [ ] MASS data conversion: Convert "prefix" in each todo to a tag, and create the tag-todo association.
- [X] BUG update filter candidate tags after new tag is added.
  - [X] re-render
    - =render-tag-filter= uses =*tag-list*= for its data
    - tag data is added in:
      - new todos: =add-tag= adds directly to =*tag-list*=
      - edit todos: (same)
  - [X] prevent duplicate display
